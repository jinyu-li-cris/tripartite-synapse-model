...
%% The second glutamate synaptic model: Based on the previous model, coupled plates of EAAT and GAT were added to study the ability of astrocytes to control E/I balance
...
Tmax=5e+4;% Time-duration of simulation; unit: ms
...
ksi=1;%  the parameter lumps,in a phenomenological way, the information on the effect of gliotransmission on synaptic release.
...
% AMPA Receptor Parameters all from Destexhe & Sejnowski (1998)
...
g_ampa=0.035;% 0.35e-6;%0.0145;%Conductance of AMPAR; unit: mS
...
g_nmda=0.035;%0.2e-6;%0.026;% Conductance of NMDAR; unit: mS
g_nmdapre=0.1;
...
alpha_gabaa=0.5;%5;% Binding of transmitter to receptor; unit: mM per ms
beta_gabaa=0.18;%;0.72% Dissociation of transmitter from receptor; unit: per ms
g_gabaa=0.0145;%2.2;%60;% Conductance of GABAR; unit: mS
g_gabaapost=0.3e-6;%%0.6e-4;
E_gabaa=-85;%GABAA reversal potential;unit: mV
...
% Kin=130; % K+ concentration in ICS, unit: mM
% Kout=3; % K+ concentration in ECS, unit: mM
...
% delta_a=17; % Modulation factor unit: mV
% Tp=293; % Temperature 
...
av_plcb=0.5;% Maximal rate of IP3 production by PLC?; unit: nM per ms
...
G_T= 1000;%1500;200;%Total vesicular gliotransmitter unit:*mmole
...
% EAAT GAT-3 NKA NCX Kir
Vol_S = 1e-17;%8.5883e-16;%Synaptic Volume unit:L
Vol_A = 7e-17;%1.885e-17;%Astrocytic Volume unit:L
S_A = 2.8274e-13;%1.4137e-13;%Astrocytic membrane surface area unit:m2
I_NKAmax = 0.1081;%Maximal NKA current unit:A/m2
K_Nai = 1.5;%NKA affinity for Na+  unit:mM
K_Ke = 10;%NKA affinity for K+ unit:mM
I_NCXmax = 0.01;%NCX max current density unit:A/m2
gamma_NCX = 0.5;%NCX partition parameter
alpha_EAAT = 2e-4;%3e-4;%EAAT scaling constant unit:A/m2
beta_EAAT = 29.2;%EAAT scaling constant unit:V−1
g_Kir = 1.44;%K+ conductance unit: S m−2
E_Kir = 0.025;%Reversal potential for Kir4.1 unit:V
g_GAT = 150;%20;%2.1e2;%GAT conductance unit: S m−2
Z_Na = 1;
Z_K = 1;
Z_Ca = 2;
Z_Glu = -1;
Z_GABA = 1;
H_syn = 5e-5;%5.0000e-07;%3.9811e-08;   %unit:mM from 2017《PHYSIOLOGY OF ASTROGLIA》
H_ast = 6e-5;%1.0000e-07;   %unit:mM from 2017《PHYSIOLOGY OF ASTROGLIA》
Cl_syn = 130;   %unit:mM adpted from 2017《PHYSIOLOGY OF ASTROGLIA》
Cl_ast = 40;   %unit:mM adpted from 2017《PHYSIOLOGY OF ASTROGLIA》
...
tauG=200;% Astrocytic intracellular glutamate clearance   unit: per ms; Ref: 2018
tauGABAsyn=200;
tauGABAast=500;
Glu_ast_norm=1.5;%0.5-5mM
G=500;%
...
x1=temp;% Temporal evolution of synaptic vesicle 1
x2=temp;% Temporal evolution of synaptic vesicle 2
...
G_syn(1)=0.01;% Glutamate concentration in cleft; unit: mM
u_syn(1)=0.8;
x_syn(1)=0.2;
...
GABA_ast(1)=70.14;%unit:mM 70.14
GABA_syn(1)=0.03;%unit:mM  0.03
Na_ast(1)=15;%16.5;%unit:mM
Na_syn(1)=90;%85; %unit:mM     
K_ast(1)=110;%unit:mM
K_syn(1)=3;%unit:mM
Glu_ast(1)=1.5;%unit:mM
% Glu_syn(1)=temp;%unit:mM
Ca_syn(1)=1.5e6;%unit:nM
...
% in 'ms.' At present the spike frequency is 5 Hz with duration of 4 ms.    
    if  t(i)>=2000 && mod(t(i),100)>=0 && mod(t(i),100)<=4
...
%     Mg_pre=1/(1+exp(-0.062*v(i))*2/3.57);
%     aI_nmdapre(i)=g_nmdapre*Mg_pre*am_nmdapre(i)*v(i);
    Ipre_gabaa(i)=g_gabaa*mpre_gabaa(i)*(v(i)-E_gabaa);% GABAAR current; unit: uA
    %% Astrocyte Processes  
...
%     JPLCd(t)=(v4+kPLCd*a)*(Cin(t)+(1-alpha)*k4)/(Cin(t)+k4); 
...
%     auer=(ac0-ca(i))/ac1;% ER Calcium concentration
    ...
    % 由于uM/s变成nM/s，要乘以1000，但是由于deltaT的时间尺度不同，从s到ms，要除以1000，正好相互抵消

    % EAAT GAT-3 NKA NCX Kir
    I_Kir(i)=g_Kir*sqrt(K_syn(i))*(Va(i)-E_Kir);
    I_NCX(i)=I_NCXmax*( (Na_ast(i)/Na_syn(i))^3*exp(gamma_NCX*F*(-Va(i))/(R*T))-(ca(i)/Ca_syn(i))*exp((gamma_NCX-1)*F*(-Va(i))/(R*T)));
    E_EAAT=(R*T/(2*F))*log((Na_syn(i)/Na_ast(i))^3*(H_syn/H_ast)*(G_syn(i)/Glu_ast(i))*(K_ast(i)/K_syn(i)));
    I_EAAT(i)=-alpha_EAAT*exp(-beta_EAAT*(Va(i)-E_EAAT));
    I_NKA(i)=I_NKAmax*((Na_ast(i))^1.5/((Na_ast(i))^1.5+(K_Nai)^1.5))*(K_syn(i)/(K_syn(i)+K_Ke));
    E_GAT(i)=(R*T/F)*log((Na_syn(i)/Na_ast(i))^2*(GABA_syn(i)/GABA_ast(i))*(Cl_syn/Cl_ast));%TRANSPORTERS | GABA Transporters: Regulation of Tonic Inhibition
    I_GAT(i)=g_GAT*(Va(i)-E_GAT(i));
     % leak current
    E_Na=(R*T/(Z_Na*F))*log(Na_syn(i)/Na_ast(i));
    I_NaL=g_Na*(Va(i)-E_Na);
    E_K=(R*T/(Z_K*F))*log(K_syn(i)/K_ast(i));
    I_KL=g_K*(Va(i)-E_K);
    E_Glu=(R*T/(Z_Glu*F))*log(G_syn(i)/Glu_ast(i));
    I_GluL=g_Glu*(Va(i)-E_Glu);
    E_GABA=(R*T/(Z_GABA*F))*log(GABA_syn(i)/GABA_ast(i));
    I_GABAL=g_GABA*(Va(i)-E_GABA);
%     E_Ca=(R*T/(Z_Ca*F))*log(Ca_syn/Ca_ast);
%     I_CaL=g_Ca*(Va-E_Ca);
    I_Naast=1.5*I_EAAT(i)+3*I_NKA(i)+3*I_NCX(i)+2*I_GAT(i)+I_NaL;%
    I_Kast=-0.5*I_EAAT(i)-2*I_NKA(i)+I_Kir(i)+I_KL;%
    I_Gluast=-0.5*I_EAAT(i)+I_GluL;
%     I_Caast=-2*I_NCX+I_CaL;
    I_GABAast=I_GAT(i);%+I_GABAL
    ...
    I_gabaa(i)=g_gabaapost*m_gabaa(i)*(V_post(i)-E_gabaa);% GABAAR current; unit: uA
    %% Bouton DF
    ...
    v(i+1)=v(i)+deltaT*(Iapp(i)-((m(i)^3)*h(i)*ina+(n(i)^4)*ik+il+Ipre_gabaa(i) ));% Membrane potential displacemnt from rest   +aI_nmdapre(i)
    c(i+1)=c(i)+deltaT*(((-ica-I_pump-I_leak)*sneuro)/(2*F*vneuro)-jpump-jleak-jchan);% Intracellular calcium 
    cer(i+1)=cer(i)+deltaT*((jchan+jleak+jpump)/c1);% ER calcium concentrationn
    
    %Fraction of Neuronal Synaptic vesicles in releasable, effectiveand inactive states respectively
    ...
    if t(i)<2000 || flag==0
        G_syn(i+1)=G_syn(1);%-degG*(G_syn(i))
        Glu_ast(i+1)=Glu_ast(1);
        GABA_ast(i+1)=GABA_ast(1);%unit:mM
        GABA_syn(i+1)=GABA_syn(1);%unit:mM
        Na_ast(i+1)=Na_ast(1);%unit:mM
        Na_syn(i+1)=Na_syn(1); %unit:mM
        K_ast(i+1)=K_ast(1);%unit:mM
        K_syn(i+1)=K_syn(1);%unit:mM
    else
        G_syn(i+1)=G_syn(i)+deltaT*( (I_Gluast*S_A/(Z_Glu*F*Vol_S)) );%-degG*(G_syn(i))
        Glu_ast(i+1)=Glu_ast(i)+deltaT*(-I_Gluast*S_A/(Z_Glu*F*Vol_A) -(Glu_ast(i)/tauG) + 0.0075 );%unit:mM 减去从星形胶质细胞释放到突触外间隙的
        GABA_ast(i+1)=GABA_ast(i)+deltaT*(-(I_GABAast)*S_A/(Z_GABA*F*Vol_A));%unit:mM
        GABA_syn(i+1)=GABA_syn(i)+deltaT*(I_GABAast*S_A/(Z_GABA*F*Vol_S));%unit:mM
        Na_ast(i+1)=Na_ast(i)+deltaT*(-(I_Naast)*S_A/(Z_Na*F*Vol_A));%unit:mM
        Na_syn(i+1)=Na_syn(i)+deltaT*(I_Naast*S_A/(Z_Na*F*Vol_S)); %unit:mM
        K_ast(i+1)=K_ast(i)+deltaT*(-(I_Kast)*S_A/(Z_K*F*Vol_A));%unit:mM
        K_syn(i+1)=K_syn(i)+deltaT*(I_Kast*S_A/(Z_K*F*Vol_S));%unit:mM
    end

    % check if spiking
    if abs(tdr-t(i))>200 && v(i)>vr     % The following formulation makes sure that a "spike" is only triggered at the first threshold crossing
            U=U0+(ksi-U0)*gama(i);
    ...
    % GABA receptor
    mpre_gabaa(i+1)=mpre_gabaa(i)+deltaT*(alpha_gabaa*GABA_syn(i)*(1-mpre_gabaa(i))-beta_gabaa*mpre_gabaa(i));% GABAAR gating variable
    ...
    % GABA receptor
    m_gabaa(i+1)=m_gabaa(i)+deltaT*(alpha_gabaa*GABA_syn(i)*(1-m_gabaa(i))-beta_gabaa*m_gabaa(i));% GABAAR gating variable
    ...
    ap(i+1)=ap(i)+deltaT*(ap_plcd-ap_mapk-ap_deg+ap_glu);% IP3 concentration  
...
%     G_T=rho_G*G;
    ...
    Ca_syn(i+1)=Ca_syn(i)+deltaT*(-2*I_NCX(i)*S_A*Vol_S/(Z_Ca*F)*10^6 - ajTRPA1(i) - Vin(i) + ajpm); %unit:mM
end
% I_Naast=1.5*I_EAAT+3*I_NKA+3*I_NCX+2*I_GAT+I_NaL;
% I_GABAast=I_GAT+I_GABAL;
% I_Kast=-0.5*I_EAAT-2*I_NKA+I_Kir+I_KL;%
%% Display Results & Total time elapsed

%detail variable presentation
figure
set(gcf,'unit','normalized','position',[0.1 0.1 0.85 0.85])
subplot(4,2,1),plot(v,'k','Linewidth',2),title('Presynaptic membrane potential','Fontsize',16,'Fontweight','bold')
set(gca,'xLim',[0,Tmax/deltaT],'FontSize',16,'LineWidth',2,'Fontweight','bold')
xticks(linspace(0,length(v)-1,3))
xticklabels(linspace(0,length(v)-1,3)/20000)
ylabel('V(mV)','FontSize',16)
hold on
subplot(4,2,2),plot(c,'k','Linewidth',2),title('Presynaptic Ca^{2+}','Fontsize',16,'Fontweight','bold')
set(gca,'xLim',[0,Tmax/deltaT],'FontSize',16,'LineWidth',2,'Fontweight','bold')
xticks(linspace(0,length(c)-1,3))
xticklabels(linspace(0,length(c)-1,3)/20000)
ylabel('[Ca^{2+}](nM)','FontSize',16)
hold on
subplot(4,2,3),plot(G_syn,'k','Linewidth',2),title('Synaptic glutamate','Fontsize',16,'Fontweight','bold')
set(gca,'xLim',[0,Tmax/deltaT],'FontSize',16,'LineWidth',2,'Fontweight','bold')
xticks(linspace(0,length(G_syn)-1,3))
xticklabels(linspace(0,length(G_syn)-1,3)/20000)
ylabel('g(mM)','FontSize',16)
hold on
subplot(4,2,4),plot(ca,'k','Linewidth',2),title('Astrocyte Ca^{2+}','Fontsize',16,'Fontweight','bold')
set(gca,'xLim',[0,Tmax/deltaT],'FontSize',16,'LineWidth',2,'Fontweight','bold')
xticks(linspace(0,length(ca)-1,3))
xticklabels(linspace(0,length(ca)-1,3)/20000)
ylabel('[Ca^{2+}](nM)','FontSize',16)
hold on
subplot(4,2,5),plot(aG_syn,'k','Linewidth',2),title('Extra-synaptic glutamate','Fontsize',16,'Fontweight','bold')
set(gca,'xLim',[0,Tmax/deltaT],'FontSize',16,'LineWidth',2,'Fontweight','bold')
xticks(linspace(0,length(aG_syn)-1,3))
xticklabels(linspace(0,length(aG_syn)-1,3)/20000)
ylabel('g(mM)','FontSize',16)
hold on
subplot(4,2,6),plot(V_post,'k','Linewidth',2),title('Postsynaptic membrane potential','Fontsize',16,'Fontweight','bold')
set(gca,'xLim',[0,Tmax/deltaT],'FontSize',16,'LineWidth',2,'Fontweight','bold')
...
hold on
subplot(4,2,7),plot(c_post,'k','Linewidth',2),title('Postsynaptic Ca^{2+}','Fontsize',16,'Fontweight','bold')
set(gca,'xLim',[0,Tmax/deltaT],'FontSize',16,'LineWidth',2,'Fontweight','bold')
xticks(linspace(0,length(c_post)-1,3))
xticklabels(linspace(0,length(c_post)-1,3)/20000)
ylabel('[Ca^{2+}](nM)','FontSize',16)
xlabel('Time (s)','FontSize',16)
hold on
...
figure
set(gcf,'unit','normalized','position',[0.1 0.1 0.85 0.85])
subplot(4,2,1),plot(Na_syn,'k','Linewidth',1),title('[Na^{+}]_{syn}','Fontsize',16,'Fontweight','bold')
set(gca,'xLim',[0,Tmax/deltaT],'FontSize',16,'LineWidth',2,'Fontweight','bold')
xticks(linspace(0,length(v)-1,3))
xticklabels(linspace(0,length(v)-1,3)/20000)
ylabel('[Na^{+}](mM)','FontSize',16)
hold on
subplot(4,2,2),plot(Na_ast,'k','Linewidth',1),title('[Na^{+}]_{ast}','Fontsize',16,'Fontweight','bold')
set(gca,'xLim',[0,Tmax/deltaT],'FontSize',16,'LineWidth',2,'Fontweight','bold')
xticks(linspace(0,length(Na_ast)-1,3))
xticklabels(linspace(0,length(Na_ast)-1,3)/20000)
ylabel('[Na^{+}](mM)','FontSize',16)
hold on
subplot(4,2,3),plot(K_syn,'k','Linewidth',1),title('[K^{+}]_{syn}','Fontsize',16,'Fontweight','bold')
set(gca,'xLim',[0,Tmax/deltaT],'FontSize',16,'LineWidth',2,'Fontweight','bold')
xticks(linspace(0,length(K_syn)-1,3))
xticklabels(linspace(0,length(K_syn)-1,3)/20000)
ylabel('[K^{+}](mM)','FontSize',16)
hold on
subplot(4,2,4),plot(K_ast,'k','Linewidth',1),title('[K^{+}]_{ast}','Fontsize',16,'Fontweight','bold')
set(gca,'xLim',[0,Tmax/deltaT],'FontSize',16,'LineWidth',2,'Fontweight','bold')
xticks(linspace(0,length(K_ast)-1,3))
xticklabels(linspace(0,length(K_ast)-1,3)/20000)
ylabel('[K^{+}](mM)','FontSize',16)
hold on
subplot(4,2,5),plot(GABA_syn*1000,'k','Linewidth',1),title('[GABA]_{syn}','Fontsize',16,'Fontweight','bold')
set(gca,'xLim',[0,Tmax/deltaT],'FontSize',16,'LineWidth',2,'Fontweight','bold')
xticks(linspace(0,length(GABA_syn)-1,3))
xticklabels(linspace(0,length(GABA_syn)-1,3)/20000)
ylabel('g(μM)','FontSize',16)
hold on
subplot(4,2,6),plot(GABA_ast,'k','Linewidth',1),title('[GABA]_{ast}','Fontsize',16,'Fontweight','bold')
set(gca,'xLim',[0,Tmax/deltaT],'FontSize',16,'LineWidth',2,'Fontweight','bold')
xticks(linspace(0,length(GABA_ast)-1,3))
xticklabels(linspace(0,length(GABA_ast)-1,3)/20000)
ylabel('g(mM)','FontSize',16)
xlabel('Time(sec)','FontSize',16)
hold on
subplot(4,2,7),plot(Glu_ast,'k','Linewidth',1),title('Astrocytic glutamate','Fontsize',16,'Fontweight','bold')
set(gca,'xLim',[0,Tmax/deltaT],'FontSize',16,'LineWidth',2,'Fontweight','bold')
xticks(linspace(0,length(Glu_ast)-1,3))
xticklabels(linspace(0,length(Glu_ast)-1,3)/20000)
ylabel('g(mM)','FontSize',16)
xlabel('Time(sec)','FontSize',16)
subplot(4,2,8),plot(E_GAT,'k','Linewidth',1),title('E_{GAT}','Fontsize',16,'Fontweight','bold')
set(gca,'xLim',[0,Tmax/deltaT],'FontSize',16,'LineWidth',2,'Fontweight','bold')
xticks(linspace(0,length(E_GAT)-1,3))
xticklabels(linspace(0,length(E_GAT)-1,3)/20000)
ylabel('V(mV)','FontSize',16)
xlabel('Time(sec)','FontSize',16)
% toc;% To display total time elapsed

data=[data;ca(1:1000000);G_syn(1:1000000);GABA_syn(1:1000000);E_GAT(1:1000000)];
